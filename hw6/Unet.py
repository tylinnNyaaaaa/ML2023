...
# ─────────────────────────────────────────────
# Step 1: Forward process (Noise Schedulers)
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# Dataset（改名避免與 torch Dataset 衝突）
# ─────────────────────────────────────────────
...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        *,
        ...
        register_buffer('loss_weight', snr.clone() / snr)
    ...
    def p_sample_loop(self, shape, return_all_timesteps=False):
        ...
        num_display = 15
        plot_timesteps = set(
            int(t) for t in torch.linspace(self.num_timesteps - 1, 0, num_display).long()
        )
        plot_imgs = []
        ...
        for t in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
            ...
            imgs.append(img)
            if t in plot_timesteps:
                plot_imgs.append((t, self.unnormalize(img.cpu())))

        # 畫 5 行 x 15 列的去噪過程
        plot_imgs.reverse()
        n_show = min(5, shape[0])
        fig, axes = plt.subplots(n_show, num_display, figsize=(num_display * 1.5, n_show * 1.5))
        for row in range(n_show):
            for col, (t, im_batch) in enumerate(plot_imgs):
                ax = axes[row][col] if n_show > 1 else axes[col]
                ax.imshow(im_batch[row].permute(1, 2, 0).clamp(0, 1))
                if row == 0:
                    ax.set_title(f't={t}', fontsize=7)
                ax.axis('off')
        plt.suptitle('Progressive Denoising (5 samples)', fontsize=13)
        plt.tight_layout()
...
# ─────────────────────────────────────────────
# Training Hyper-parameters（Strong Baseline）
# ─────────────────────────────────────────────
...
# ★ Strong baseline 改動
batch_size = 8               # 模型變大，降低 batch size 避免 OOM
...
channels = 64                # 16 → 64
dim_mults = (1, 2, 4, 8)     # 多一層

timesteps = 1000             # 100 → 1000
...
# 顯示 5 張圖的去噪過程（Gradescope Q1）
trainer.ema.ema_model.eval()
with torch.no_grad():
    _ = trainer.ema.ema_model.sample(batch_size=5)
...
# 打包
import subprocess
subprocess.run(['tar', '-zcf', 'submission.tgz', '-C', './submission', '.'])
